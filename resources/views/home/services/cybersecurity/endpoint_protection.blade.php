@extends('home.includes.layout')
{{-- @section('title', 'Threat Detection & Endpoint Security Services | Mounteko') --}}
@section('title', 'Threat Detection & Endpoint Security Services | Mounteko')
{{-- @section('meta-description',
'24/7 threat detection, prevention, and endpoint security for growing businesses — stop attacks before they reach your team.') --}}
@section('meta-description', '24/7 threat detection, prevention, and endpoint security for growing businesses — stop attacks before they reach your team.')
@section('header-area')
    <link rel="stylesheet" href="{{ asset('home/assets/homepage/css/css-style.min.css') }}">
    <link rel="stylesheet" href="{{ asset('home/assets/homepage/css/css-icon.min.css') }}">
    <link rel="stylesheet" href="{{ asset('home/assets/homepage/css/new_home_custom.css') }}">
    <link rel="stylesheet" href="{{ asset('home/assets/homepage/css/cybersecurity_services.css') }}">
    <style>
        /* Decrease heading sizes across the entire page */
        .h2-content h2 {
            font-size: 1.8rem !important;
        }

        .s3-content h2,
        .s4-title,
        .s5-title,
        .s7-title,
        .s8-title {
            font-size: 1.6rem !important;
        }

        .s3-feature-text h5 {
            font-size: 1.1rem !important;
        }

        .s4-card h4 {
            font-size: 1.1rem !important;
        }

        .s7-service-block h5 {
            font-size: 1.1rem !important;
        }

        .s5-item h5 {
            font-size: 13px !important;
        }
    </style>
@endsection

@section('content')
    <main id="main">

        <!-- HEADER 2 (HERO) -->
        <section class="header2-section">
            <div class="h2-container">
                <div class="h2-content">
                    <h2>Advanced Threat Detection & Endpoint Security to Protect Your <span>Business</span></h2>
                    <p>Proactive threat detection, prevention, and compliance-driven endpoint protection services to
                        safeguard your data, networks, and users.</p>
                    <div class="h2-btns" style="display: flex; gap: 15px; flex-wrap: wrap;">
                        <a href="{{ route('contactUs') }}" class="btn-blue-grad">Get a Free Security Assessment</a>
                        <a href="{{ route('contactUs') }}" class="btn-blue-grad"
                            style="background: transparent; border: 2px solid white;">Talk to a Security Expert</a>
                    </div>
                </div>
                <div class="h2-visual">
                    <img src="https://images.unsplash.com/photo-1558494949-ef010cbdcc31?auto=format&fit=crop&q=80&w=600"
                        class="dashboard-img" alt="Endpoint Security Dashboard">
                    <div class="shield-overlay" style="display: flex; justify-content: center; align-items: center;">
                        <i class="bi bi-shield-fill-exclamation"
                            style="font-size: 150px; color: white; filter: drop-shadow(0 0 30px rgba(77, 163, 255, 0.9));"></i>
                    </div>
                </div>
            </div>

            <div class="h2-bottom-bar">
                <div class="h2-bottom-text">
                    <div class="h2-line"></div>
                    Without proactive endpoint security and 24/7 threat detection, businesses remain vulnerable 24x7.
                    <div class="h2-line"></div>
                </div>
            </div>
        </section>

        <!-- PREMIUM SERVICES MENU PILLS -->
        @include('home.includes.cybersecurity_pills')

        <!-- SECTION 3 (FEATURES / ABOUT US) -->
        <section class="section3-features">
            <div class="s3-container">
                <div class="s3-visual">
                    <div class="img-tall-wrap">
                        <img src="https://images.unsplash.com/photo-1573164713714-d95e436ab8d6?auto=format&fit=crop&q=80&w=600"
                            class="s3-img" style="height: 380px;" alt="Threat Intelligence Center">
                    </div>
                    <div class="img-stack-wrap">
                        <img src="https://images.unsplash.com/photo-1563986768609-322da13575f3?auto=format&fit=crop&q=80&w=400"
                            class="s3-img" style="height: 180px;" alt="Endpoint Devices">
                        <img src="https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&q=80&w=400"
                            class="s3-img" style="height: 180px;" alt="Security Server">
                    </div>
                    <div class="badge-circular">
                        <div class="badge-inner">
                            <i class="bi bi-shield-check fs-5 mb-1"></i>
                            Get free<br>cyber security
                        </div>
                    </div>
                </div>

                <div class="s3-content">
                    <div class="s3-tag">
                        <i class="bi bi-eye-fill"></i> ABOUT US
                    </div>
                    <h2>Your trusted partner in <span>threat detection & endpoint security solutions</span></h2>
                    <p>We provide reliable, cutting-edge managed threat detection and response services to protect your
                        digital assets from evolving cyber risks, ensuring operational safety and peace of mind.</p>

                    <div class="s3-feature-box">
                        <div class="s3-feature-img">
                            <img src="https://images.unsplash.com/photo-1573164713714-d95e436ab8d6?auto=format&fit=crop&q=80&w=300"
                                alt="Security Specialist">
                        </div>
                        <div class="s3-feature-text">
                            <h5>24/7 SOC monitoring & real-time threat detection</h5>
                            <p>Stopping attacks at the perimeter and on the endpoint with our comprehensive monitoring.</p>
                        </div>
                    </div>

                    <ul class="s3-checklist">
                        <li><i class="bi bi-check-circle-fill"></i> Advanced Threat Detection and Prevention</li>
                        <li><i class="bi bi-check-circle-fill"></i> Endpoint Protection & Access Control Management</li>
                        <li><i class="bi bi-check-circle-fill"></i> Security Awareness & Phishing Simulation Training</li>
                    </ul>

                    <div class="s3-footer">
                        <div class="s3-phone">
                            <div class="s3-phone-icon">
                                <i class="bi bi-telephone-fill"></i>
                            </div>
                            <strong>(855) 526-7890</strong>
                        </div>
                        <a href="{{ route('contactUs') }}" class="btn-s3-grad">
                            More About Our Services <i class="bi bi-arrow-right-short"></i>
                        </a>
                    </div>
                </div>
            </div>
        </section>

        <!-- SECTION 4 (WHAT WE PROVIDE) -->
        <section class="section4-services">
            <div class="container">
                <span class="s4-tagline">WHAT WE PROVIDE</span>
                <h2 class="s4-title">Endpoint Security & Threat Detection for <span>Today's Threats</span>
                </h2>
                <p class="s4-desc">Protect your business from ever-evolving cyber risks with our end-to-end managed threat
                    detection and response solutions.</p>

                <div class="s4-grid">
                    <div class="s4-card card-network">
                        <div class="s4-icon-box"><i class="bi bi-shield-slash-fill"></i></div>
                        <h4>Advanced Threat Protection</h4>
                        <p>Deploying next-gen AI models and behavioral analytics to identify and block sophisticated malware
                            and ransomware before execution.</p>
                        <a href="{{ route('contactUs') }}" class="btn-learn">Learn More</a>
                    </div>
                    <div class="s4-card card-data">
                        <div class="s4-icon-box"><i class="bi bi-laptop-fill"></i></div>
                        <h4>Endpoint Security Solutions</h4>
                        <p>Securing all employee devices (laptops, mobile, servers) with unified endpoint protection
                            services and automated patch management.</p>
                        <a href="{{ route('contactUs') }}" class="btn-learn">Learn More</a>
                    </div>
                    <div class="s4-card card-identity">
                        <div class="s4-icon-box"><i class="bi bi-binoculars-fill"></i></div>
                        <h4>Managed Detection & Response</h4>
                        <p>24/7 threat hunting and investigation by our SOC team to contain and remediate active threats in
                            your environment.</p>
                        <a href="{{ route('contactUs') }}" class="btn-learn">Learn More</a>
                    </div>
                    <div class="s4-card card-monitor">
                        <div class="s4-icon-box"><i class="bi bi-display-fill"></i></div>
                        <h4>SOC Monitoring Services</h4>
                        <p>Continuous security event monitoring with advanced SIEM systems for real-time alerting and
                            compliance reporting.</p>
                        <a href="{{ route('contactUs') }}" class="btn-learn">Learn More</a>
                    </div>
                    <div class="s4-card card-backup">
                        <div class="s4-icon-box"><i class="bi bi-tools"></i></div>
                        <h4>Incident Response & Recovery</h4>
                        <p>Rapid containment and recovery plans to minimize downtime and data loss during a confirmed breach
                            or ransomware attack.</p>
                        <a href="{{ route('contactUs') }}" class="btn-learn">Learn More</a>
                    </div>
                    <div class="s4-card card-config">
                        <div class="s4-icon-box"><i class="bi bi-gear-wide-connected"></i></div>
                        <h4>Security Configuration Hardening</h4>
                        <p>Hardening endpoint configurations and applying CIS benchmarks across your entire infrastructure
                            to reduce attack surface.</p>
                        <a href="{{ route('contactUs') }}" class="btn-learn">Learn More</a>
                    </div>
                </div>
            </div>
        </section>

        <!-- SECTION 5 (WHY CHOOSE US) -->
        <section class="section5-why-us">
            <div class="container">
                <h2 class="s5-title">Why Choose Our Threat Detection & Endpoint Security?</h2>
                <div class="s5-row" style="flex-wrap: wrap; justify-content: center;">
                    <div class="s5-item" style="flex: 0 0 calc(16.66% - 10px); min-width: 150px; text-align: center;">
                        <div class="s5-icon-circle"><i class="bi bi-activity"></i></div>
                        <h5
                            style="color: #1a237e; font-size: 14px; font-weight: 700; margin-bottom: 5px; white-space: nowrap; letter-spacing: -0.3px;">
                            Proactive Threat Detection</h5>
                    </div>
                    <div class="s5-item" style="flex: 0 0 calc(16.66% - 10px); min-width: 150px; text-align: center;">
                        <div class="s5-icon-circle"><i class="bi bi-laptop"></i></div>
                        <h5
                            style="color: #1a237e; font-size: 14px; font-weight: 700; margin-bottom: 5px; white-space: nowrap; letter-spacing: -0.3px;">
                            Endpoint Security</h5>
                    </div>
                    <div class="s5-item" style="flex: 0 0 calc(16.66% - 10px); min-width: 150px; text-align: center;">
                        <div class="s5-icon-circle"><i class="bi bi-clock-history"></i></div>
                        <h5
                            style="color: #1a237e; font-size: 14px; font-weight: 700; margin-bottom: 5px; white-space: nowrap; letter-spacing: -0.3px;">
                            24/7 SOC Monitoring</h5>
                    </div>
                    <div class="s5-item" style="flex: 0 0 calc(16.66% - 10px); min-width: 150px; text-align: center;">
                        <div class="s5-icon-circle"><i class="bi bi-lightning-charge-fill"></i></div>
                        <h5
                            style="color: #1a237e; font-size: 14px; font-weight: 700; margin-bottom: 5px; white-space: nowrap; letter-spacing: -0.3px;">
                            Rapid Incident Response</h5>
                    </div>
                    <div class="s5-item" style="flex: 0 0 calc(16.66% - 10px); min-width: 150px; text-align: center;">
                        <div class="s5-icon-circle"><i class="bi bi-shield-check"></i></div>
                        <h5
                            style="color: #1a237e; font-size: 14px; font-weight: 700; margin-bottom: 5px; white-space: nowrap; letter-spacing: -0.3px;">
                            Managed Detection & Response</h5>
                    </div>
                    <div class="s5-item"
                        style="flex: 0 0 calc(16.66% - 10px); min-width: 150px; text-align: center; border-right: none;">
                        <div class="s5-icon-circle"><i class="bi bi-person-check-fill"></i></div>
                        <h5
                            style="color: #1a237e; font-size: 14px; font-weight: 700; margin-bottom: 5px; white-space: nowrap; letter-spacing: -0.3px;">
                            Single Partner Solution</h5>
                    </div>
                </div>
            </div>
        </section>

        <!-- SECTION 7 (WORLD SECURITY) -->
        <section class="section7-securing">
            <div class="s7-container">
                <div class="s7-top-row">
                    <div class="s7-left">
                        <div class="s7-tag">
                            <i class="bi bi-shield-check"></i> OUR SECURITY
                        </div>
                        <h2 class="s7-title">Securing your digital world together with <span class="together">proactive
                                endpoint defense</span></h2>

                        <div class="s7-service-block">
                            <h5>Real-Time Threat Detection and Prevention</h5>
                            <p>Our advanced threat detection services continuously analyze network traffic, user behavior,
                                and system activity to detect and respond to potential security threats as they occur. We
                                stop attacks at the perimeter and on the endpoint.</p>
                        </div>

                        <div class="s7-service-block">
                            <h5>Compliance And Risk Management:</h5>
                            <p>Our endpoint security solutions ensure compliance with industry regulations by continuously
                                monitoring for vulnerabilities, misconfigurations, and unauthorized access attempts across
                                all managed devices.</p>
                        </div>
                    </div>

                    <div class="s7-right">
                        <div class="s7-image-wrap">
                            <img src="https://images.unsplash.com/photo-1560264280-88b68371db39?auto=format&fit=crop&q=80&w=800"
                                alt="Security Operations Center">
                        </div>

                        <div class="s7-service-block">
                            <h5>Endpoint Protection Services:</h5>
                            <p>Our endpoint protection services continuously analyze process behavior and file activity to
                                detect and respond to potential zero-day exploits, ransomware encryption events, and
                                memory-based attacks as they occur.</p>
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <!-- BOTTOM BAR - 3 MINI FEATURES -->
        <section style="background-color: #f0f5fb; padding: 50px 0;">
            <div class="container">
                <div class="row g-4">
                    <div class="col-lg-4 col-md-6 text-center">
                        <div
                            style="background: white; padding: 30px; border-radius: 10px; height: 100%; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                            <i class="bi bi-shield-lock-fill"
                                style="font-size: 2rem; color: var(--primary-color); margin-bottom: 15px;"></i>
                            <h5 style="font-weight: 700; color: #1a237e; margin-bottom: 15px; font-size: 1.1rem;">ENDPOINT
                                SECURITY</h5>
                            <p style="color: #555; font-size: 14px; margin: 0;">Protecting systems and data from
                                unauthorized access, malware, and advanced persistent threats.</p>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 text-center">
                        <div
                            style="background: white; padding: 30px; border-radius: 10px; height: 100%; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                            <i class="bi bi-search"
                                style="font-size: 2rem; color: var(--primary-color); margin-bottom: 15px;"></i>
                            <h5 style="font-weight: 700; color: #1a237e; margin-bottom: 15px; font-size: 1.1rem;">THREAT
                                DETECTION & HUNTING</h5>
                            <p style="color: #555; font-size: 14px; margin: 0;">Proactive identification of hidden attackers
                                and suspicious behavior across your digital environment.</p>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 text-center">
                        <div
                            style="background: white; padding: 30px; border-radius: 10px; height: 100%; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                            <i class="bi bi-activity"
                                style="font-size: 2rem; color: var(--primary-color); margin-bottom: 15px;"></i>
                            <h5 style="font-weight: 700; color: #1a237e; margin-bottom: 15px; font-size: 1.1rem;">MANAGED
                                DETECTION & RESPONSE</h5>
                            <p style="color: #555; font-size: 14px; margin: 0;">24/7 expert monitoring and remediation to
                                stop threats before they impact operations.</p>
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <!-- SECTION 8 (MODERN CTA) -->
        <section class="section8-cta">
            <div class="s8-container">
                <div class="s8-box">
                    <div class="s8-left">
                        <h2 class="s8-title">Protect your business from sophisticated cyber threats today!</h2>
                        <span
                            style="color: rgba(255,255,255,0.8); display: block; margin-bottom: 25px; font-size: 1.1rem;">Don't
                            let hidden vulnerabilities and unpatched systems compromise your entire network
                            infrastructure.</span>
                        <div class="s8-contacts">
                            <div class="s8-contact-item">
                                <i class="bi bi-telephone-outbound"></i>
                                <div class="s8-contact-info">
                                    <span>Get contact now</span>
                                    <strong>(855) 526-7890</strong>
                                </div>
                            </div>
                            <div class="s8-sep"></div>
                            <div class="s8-contact-item">
                                <i class="bi bi-envelope-at"></i>
                                <div class="s8-contact-info">
                                    <span>Sent e-mail</span>
                                    <strong>info@mounteko.com</strong>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="s8-right">
                        <div class="s8-image-wrap">
                            <img src="https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&q=80&w=800"
                                alt="Cyber Security Solutions">
                        </div>
                        <div class="s8-arrow">
                            <svg viewBox="0 0 100 50">
                                <path d="M10,40 Q40,40 40,20 T70,10 M60,5 L70,10 L65,20" />
                            </svg>
                        </div>
                    </div>
                </div>
            </div>
        </section>

    </main>
@endsection